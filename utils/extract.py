...
def _retrieveTrialSlice(data, trialIndex, type="signal"):
    if type == "signal":
        return slice(
            int(data.trials[trialIndex] + data.trialSignalOffset[0] * data.sampling_freq),
            int(data.trials[trialIndex] + data.trialSignalOffset[1] * data.sampling_freq),
        )

    elif type == "force":
        return slice(
            int(data.trials[trialIndex] + data.trialForceOffset[0] * data.sampling_freq),
            int(data.trials[trialIndex] + data.trialForceOffset[1] * data.sampling_freq),
        )

    elif type == "background":
        return slice(
            int(data.trials[trialIndex] + data.trialBgOffset[0] * data.sampling_freq),
            int(data.trials[trialIndex] + data.trialBgOffset[1] * data.sampling_freq),
        )
...
def getTrials(data, filtered=None, background=False):
    ...
    for pair in data.electrodePairList:
        data.channel.append(_processData(raw_data[:, pair[0]] - raw_data[:, pair[1]]))
    ...
    if background:
        return [
            (
                data.channel[0][_retrieveTrialSlice(data, i % 3, "background")],
        ...
        return [
            (
                data.channel[0][_retrieveTrialSlice(data, i, "signal")],
